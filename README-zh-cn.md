...
完整数据可以查看机器生成的[信源健康报告](data/reports/source-health.json)、[系统能力评测](data/reports/system-evaluation.json)、[数据源与评分原则](docs/SOURCES.md)和[能力图谱](docs/CAPABILITIES.md)。CI 会恢复完整仓库快照，用新评测对比上一提交；分数或证据覆盖回退会直接失败，并把最高优先级证据缺口作为持续提升队列写入 Actions Summary。
